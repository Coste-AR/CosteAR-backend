...
import { registerInventarioRoutes } from './routes/inventario.routes.js';
...
export async function buildApp(): Promise<FastifyInstance> {
  ...
  // La política centralizada permite los dominios propios exactos, previews de
  // Vercel y localhost aun si CORS_ORIGIN quedó mal configurada en el deploy.
  await app.register(cors, {
    origin: createCorsOriginHandler(allowedOrigins, app.log),
    credentials: true,
  ...
  await app.register(
    async (api) => {
      ...
      await registerInventarioRoutes(api);
