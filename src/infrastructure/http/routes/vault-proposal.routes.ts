...
export async function registerVaultProposalRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/vault/proposals', { preHandler: [authenticate, requireRole('SUPER_ADMIN')] }, async (request, reply) => {
    const list = await service.listPending();
    return reply.status(200).send({ data: list });
  });

  app.patch('/vault/proposals/:id', { preHandler: [authenticate, requireRole('SUPER_ADMIN')] }, async (request, reply) => {
  ...
  app.post('/vault/proposals/:id/approve', { preHandler: [authenticate, requireRole('SUPER_ADMIN')] }, async (request, reply) => {
    const { id } = idParam.parse(request.params);
    const approved = await service.approveProposal(request.authUser!.id, id);
    return reply.status(200).send({ data: approved });
  });

  app.post('/vault/proposals/:id/reject', { preHandler: [authenticate, requireRole('SUPER_ADMIN')] }, async (request, reply) => {
    const { id } = idParam.parse(request.params);
