...
export class PaqueteRubroService {
  ...
  async resolve(userId: string, category: string, scope: PackageScope = {}): Promise<ResolvedPackage> {
    ...
    for (const row of eligible) {
      Object.assign(result, { nombreProducto: row.nombreProducto, lexicon: row.lexicon, icons: row.icons, variants: row.variants, seedParameters: row.seedParameters, alertRules: row.alertRules, screens: row.screens, modulos: row.modulos });
