...
|**PR-05**|**El agente no mergea — tampoco con `gh pr merge --auto`.** Desde el 30-08-2026 mergea `.github/workflows/auto-merge.yml`, y sólo con todos los checks en verde (cero checks no cuenta) **y** la etiqueta `auto-merge`, que pone Santiago: es el juicio humano que **reemplaza al review**. **Una rama de trabajo entra a `dev` con SQUASH**: un PR = un commit, `dev` se lee como una lista de cambios y deshacerlo es un `git revert` de un commit. **Las PROMOCIONES (`dev→staging`, `staging→main`) van con MERGE COMMIT, nunca squash**: el squash crea una identidad nueva y git deja de reconocer la historia compartida — el PR #125 existió solo para resolver un conflicto fantasma causado por eso. Canónico: `CosteAR-os/ORQUESTACION.md`.|
